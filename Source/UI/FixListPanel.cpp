...
void FixListPanel::paint(juce::Graphics& g)
{
    ...
    const auto* cards = model != nullptr ? &model->fixList : nullptr;
    ...
    if (!model || cards == nullptr || cards->empty())
    {
        ...
        if (model != nullptr)
        {
            ...
            else if (!model->hasLiveSignal)
            {
                message = "No valid program material yet. Play audio to populate the fix list.";
            }
            else if (!model->hasScoredResult)
            {
                message = "Live metrics are updating. Reference-weighted scoring is still warming, but measurement-based fixes remain active.";
            }
            else
            {
                message = "No current issues. Hold the current balance or make smaller moves.";
            }
        }
        else
        {
