                                   ...
                                   float playbackSeconds) const
{
    ...
    const float targetCrest = std::isfinite(targetFeature.crestDb) ? targetFeature.crestDb
                                                                   : fallbackFeature.crestDb;
    const float targetTransient =
        std::isfinite(targetFeature.transientDensity) ? targetFeature.transientDensity
                                                      : fallbackFeature.transientDensity;
    const float targetSide = std::isfinite(targetFeature.sideEnergy) ? targetFeature.sideEnergy
                                                                     : fallbackFeature.sideEnergy;
    const float targetCorrelation =
        std::isfinite(targetFeature.correlation) ? targetFeature.correlation
                                                 : fallbackFeature.correlation;
    const float targetMud = std::isfinite(targetFeature.mud) ? targetFeature.mud : fallbackFeature.mud;
    const float targetHarshness =
        std::isfinite(targetFeature.harshness) ? targetFeature.harshness : fallbackFeature.harshness;
    const float targetAir = std::isfinite(targetFeature.air) ? targetFeature.air : fallbackFeature.air;
