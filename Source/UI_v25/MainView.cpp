...
#include "coresynth/advisory_layer/api_key_store.hpp"
#include "coresynth/advisory_layer/openai_adviser.hpp"
#include "coresynth/audiosuite_core/system_contract.hpp"
...
namespace
{
constexpr auto kMissingKeyFeedback = "OpenAI API key missing — AudioSuite chat unavailable";
constexpr auto kOfflineFeedback = "OpenAI request failed — AudioSuite chat unavailable";
...
{
    return coresynth::advisory_layer::ApiKeyStore::loadOpenAiKey().empty()
...
juce::String firstNonEmptyLine(const std::vector<std::string>& values, const juce::String& fallback = {})
{
    for (const auto& value : values)
    {
        const auto candidate = juce::String(value).trim();
...
int uiVariant()
{
#if AUDIOSYNTH_UI_VARIANT == 2
    return 2;
#elif AUDIOSYNTH_UI_VARIANT == 3
    return 3;
#elif AUDIOSYNTH_UI_VARIANT == 4
    return 4;
#else
    return 1;
#endif
...
MainView::MainView(CoreSynthAudioProcessor& p)
    : processor(p),
      advisory(std::make_unique<coresynth::advisory_layer::OpenAiAdviser>(
          coresynth::advisory_layer::ApiKeyStore::loadOpenAiKey(),
          kLockedChatModel,
          "https://api.openai.com/v1/chat/completions"))
{
    activeLayoutVariant = uiVariant();
    ...
    chatPrompt.setTextToShowWhenEmpty("Ask AudioSuite... (session history only)",
    ...
    if (!coresynth::advisory_layer::ApiKeyStore::loadOpenAiKey().empty())
    {
    ...
    apiKeySaveButton.onClick = [this]
    {
        ...
        if (coresynth::advisory_layer::ApiKeyStore::saveOpenAiKey(key))
        {
            apiKeyEntry.clear();
    ...
    {
        (void)coresynth::advisory_layer::ApiKeyStore::clearOpenAiKey();
        apiKeyEntry.clear();
    ...
    compareMenuLabel.setText(
        "COMPARE MENU\nLoad premix and mixed renders, then run DSP-linked A/B analysis.",
        juce::dontSendNotification);
    ...
    {
        if (!hasComparePremix || !hasCompareMix)
        {
            compareMenuStatus.setText("Capture both premix and mix snapshots before comparing.",
                                      juce::dontSendNotification);
            return;
        }
        compareMenuStatus.setText("Compare snapshots locked; switch TopBar to Compare mode.",
                                  juce::dontSendNotification);
    };
    compareMenuStatus.setText("No files loaded.", juce::dontSendNotification);
    ...
    comparePremixModel.prepare(kUiRefreshHz);
    compareMixModel.prepare(kUiRefreshHz);
...
void MainView::appendChatHistoryEntry(const juce::String& entry)
{
    ...
    while (chatHistoryEntries.size() > 7)
    {
        chatHistoryEntries.pop_front();
    }
...
void MainView::timerCallback()
{
    ...
    if (got)
    {
        ...
        // automatic advisory now manual; live buffer only feeds model.
    }
    flushDebugLog(false);

    model.advance();

    if (activeModePane == ModePane::Compare && hasComparePremix && hasCompareMix)
    {
        approvalHalo.setDisplayMode(ApprovalHalo::DisplayMode::Compare);
        approvalHalo.setComparisonModel(&comparePremixModel);
    }
    else if (activeModePane == ModePane::Reference)
    {
        approvalHalo.setDisplayMode(ApprovalHalo::DisplayMode::Reference);
        approvalHalo.setComparisonModel(nullptr);
    }
    else
    {
        approvalHalo.setDisplayMode(ApprovalHalo::DisplayMode::Live);
        approvalHalo.setComparisonModel(nullptr);
    }
...
void MainView::captureCompareSnapshot(bool isPremix)
{
    if (!hasLastFrame)
    {
        compareMenuStatus.setText("Awaiting live buffer to capture snapshot.",
                                  juce::dontSendNotification);
        return;
    }
    ...
    if (isPremix)
    {
        comparePremixFrame = lastFrame;
        comparePremixModel.updateFrom(comparePremixFrame);
        comparePremixModel.advance();
        hasComparePremix = true;
        compareMenuStatus.setText("Premix snapshot captured from the live buffer.",
                                  juce::dontSendNotification);
    }
    else
    {
        compareMixFrame = lastFrame;
        compareMixModel.updateFrom(compareMixFrame);
        compareMixModel.advance();
        hasCompareMix = true;
        compareMenuStatus.setText("Mix snapshot captured from the live buffer.",
                                  juce::dontSendNotification);
    }
}
        ...
        {"analysis",
         {genreFromIndex(frame.genreIndex),
          ...
          std::string("preserve punch and emotional impact; contract=") +
              coresynth::audiosuite_core::contract::kSystemId +
              "; metrics=" + coresynth::audiosuite_core::contract::kMetricMathPack,
          "vst-session",
          promptText,
          ...
          liveAnalysisJson,
          liveFixJson},
         [this, appendToChat](std::optional<coresynth::advisory_layer::AdvisoryOutput> output)
         {
             ...
             if (!output)
             {
                 juce::MessageManager::callAsync(
                     [this, appendToChat]
                     {
                         ...
                         fixList.setModel(model);
                         repaint();
                         appendChatHistoryEntry("AudioSuite: " + message);
                     });
                 return;
             }
             ...
             juce::MessageManager::callAsync(
                 ...
                 {
                     appendChatHistoryEntry((appendToChat ? "AudioSuite: " : "AudioSuite [Live]: ") +
                                            juce::String(value.summary30s));

                     model.statusText = appendToChat ? "INSIGHT READY" : "LIVE BUFFER LINKED";
                     if (!value.issues.empty())
                     {
                         for (std::size_t i = 0; i < value.issues.size() && i < 3; ++i)
                         {
                             const auto& issue = value.issues[i];
                             const auto nextStep = firstNonEmptyLine(issue.tryFirst);
                             const auto why = juce::String(issue.whyItMatters).trim().isNotEmpty()
                                                  ? juce::String(issue.whyItMatters)
                                                  : juce::String(value.summary30s);
                             juce::String impact = juce::String(issue.severity).trim();
                             if (!issue.evidencePointers.empty())
                             {
                                 if (impact.isNotEmpty())
                                 {
                                     impact << " | ";
                                 }
                                 impact << juce::String(issue.evidencePointers.front());
                             }
                             model.fixList.push_back({
                                 "ai_fix_" + juce::String(static_cast<int>(juce::Time::getMillisecondCounter())) + "_" + juce::String(static_cast<int>(i + 1)),
                                 juce::String(issue.summary),
                                 why,
                                 nextStep,
                                 impact,
                                 static_cast<float>(value.signalClarity)});
                         }
                     }
                     else
                     {
                         const std::size_t count = juce::jmin<std::size_t>(3, value.top3Fixes.size());
                         for (std::size_t i = 0; i < count; ++i)
                         {
                             model.fixList.push_back({
                                 "ai_fix_" + juce::String(static_cast<int>(juce::Time::getMillisecondCounter())) + "_" + juce::String(static_cast<int>(i + 1)),
                                 juce::String(value.top3Fixes[i]),
                                 juce::String(value.summary30s),
                                 value.safePath.empty() ? juce::String(value.boldPath)
                                                        : juce::String(value.safePath),
                                 juce::String(value.details),
                                 static_cast<float>(value.signalClarity)});
                         }
                         if (count == 0 && !juce::String(value.summary30s).trim().isEmpty())
                         {
                             model.fixList.push_back({
                                 "ai_fix_" + juce::String(static_cast<int>(juce::Time::getMillisecondCounter())),
                                 juce::String(value.summary30s),
                                 juce::String(value.details),
                                 juce::String(value.safePath),
                                 juce::String(value.boldPath),
                                 static_cast<float>(value.signalClarity)});
                         }
                     }

                    if (model.fixList.size() > 7)
                    {
                        model.fixList.erase(model.fixList.begin(), model.fixList.begin() + static_cast<long>(model.fixList.size() - 7));
                    }
