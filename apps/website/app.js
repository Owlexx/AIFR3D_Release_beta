const config = window.DAWAI_CONFIG || {};
...
const SITE_TEXT_DEFAULTS = Object.freeze({
  ...
  brandSubtitle: "AIFR3D | Mix Intelligence | Licensing",
  ...
  heroCopy: "AIFR3D combines live diagnostics, reference matching, stereo field intelligence, tonal balance insight, loudness control, and Aifred Mix Tips inside one focused North3rnLight3r workflow for artists, producers, and engineers.",
...
function loadUnlockState() {
  try {
    const raw = localStorage.getItem("dawai_install_unlock");
    if (!raw) {
      return;
    }
  ...
  const installCards = [
    {
      ...
      description: "Linux VST3 installer for the current AIFR3D beta with live diagnostics, stereo field analysis, reference matching, and Aifred Mix Tips.",
    ...
    },
    {
      ...
      description: "Linux standalone build for the current AIFR3D beta with the same analysis and advisory workflow as the plugin.",
      price: BETA_FREE_TRIAL_LABEL,
      assetId: "install-standalone.sh",
      productKey: "standalone"
    },
    ...
    },
    {
      ...
      description: "Minimum Windows and Linux targets for stable AIFR3D playback, analysis, and UI performance.",
      price: "",
