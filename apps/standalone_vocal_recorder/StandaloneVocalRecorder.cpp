...
void StandaloneVocalRecorderComponent::getNextAudioBlock (const juce::AudioSourceChannelInfo& bufferToFill)
{
    ...
    audiosynth::dsp::AnalysisFrame currentFrame = audioProcessor.getAnalysisFrame();
...
void StandaloneVocalRecorderComponent::sendVocalDspMetrics(const audiosynth::dsp::AnalysisFrame& frame)
{
    if (webSocket && webSocket->isConnected())
    {
