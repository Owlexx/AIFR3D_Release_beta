...
const EXTERNAL_CATALOG_DIR_CANDIDATES = Array.from(new Set([
  CATALOG_DIR,
  String(process.env.CORESYNTH_EXTERNAL_CATALOG_DIR || "").trim(),
  LEGACY_CATALOG_DIR,
...
const BRAIN_CONFIG_FILE = path.join(ROOT_DIR, "assets/audiosuite_brain/brain_config.json");

const PORT = Number(process.env.PORT || 8787);
const ADMIN_USER_ENV = String(process.env.ADMIN_USER || process.env.CORESYNTH_ADMIN_USER || "").trim();
const ADMIN_PASS_ENV = String(process.env.ADMIN_PASS || process.env.CORESYNTH_ADMIN_PASS || "").trim();
const ADMIN_PASS_HASH_ENV = String(process.env.ADMIN_PASS_HASH || process.env.CORESYNTH_ADMIN_PASS_HASH || "").trim();
const API_TOKEN = String(process.env.CORESYNTH_API_TOKEN || "").trim();
...
const DEFAULT_ONLINE_API = String(process.env.CORESYNTH_DEFAULT_API_URL || "https://www.north3rnlight3r.com").trim();
...
const PROMO_CODES_DISABLED_MESSAGE = "Promo codes are disabled for AudioSuite 2.2.4 Beta.";
const BETA_FREE_TRIAL_MESSAGE = "AudioSuite 2.2.4 Beta is free to try for a limited time. Use the installer download instead of checkout.";
...
const PROMO_CODES_FILE_ENV = String(process.env.CORESYNTH_PROMO_CODES_FILE || "").trim();
const LOCAL_PROMO_FILE_CANDIDATES = [
  PROMO_CODES_FILE_ENV,
  path.join(process.env.HOME || "", "Documents/AudioSuite_promo_codes_2.2.4_beta.json"),
  "/home/north3rnlight3r/Documents/AudioSuite_promo_codes_2.2.4_beta.json",
...
const DEFAULT_AudioSuite_BRAIN = {
  brain_id: "audiosuite_brain_v2_2_4",
  version: "2.2.4",
  identity: {
    name: "AudioSuite",
...
const AudioSuite_BRAIN = loadAudioSynthBrain();
...
const DEFAULT_CONTENT = {
  ...
  products: [
    {
      sku: "software_vst3",
      title: "AudioSuite VST Beta 2.2.4",
...
const DEFAULT_STORE_PRODUCTS = [
  {
    sku: "software_vst3",
    name: "AudioSuite VST Beta 2.2.4",
...
const ACTION_COMMANDS = {
  ...
  "analysis.engine.verify": "ctest --test-dir build --output-on-failure -R 'profile_cache_test|genre_detector_test|audiosuite_fix_list_reactivity_smoke'"
};
...
function configuredOpenAiModelList() {
  const raw = String(
    process.env.CORESYNTH_OPENAI_MODEL_ALLOWLIST ||
    process.env.OPENAI_MODEL_ALLOWLIST ||
    process.env.CORESYNTH_OPENAI_MODELS ||
    ""
  ).trim();
  if (!raw) {
...
function configuredOpenAiPrimaryModel() {
  const requested = normalizeModelName(
    process.env.CORESYNTH_OPENAI_PRIMARY_MODEL ||
...
async function emitChatWebhook(eventType, payload) {
  ...
  if (settings.webhook.secret) {
    headers["X-AudioSuite-Webhook-Signature"] = hmacHex(settings.webhook.secret, body);
  }
...
function buildSystemPrompt(personalityMode, agentModes, chatSettings = DEFAULT_CHAT_SETTINGS) {
  ...
  return [
    `You are ${brain?.identity?.name || "AudioSuite"}, a structured production intelligence system.`,
...
function loadPaymentUnlockBundle() {
  const fallback = {
    ...
    note: "AudioSuite 2.2.4 Beta software installers are free to try for a limited time."
  };
  try {
...
function promoProductDownloadPath(product) {
  if (product === "vst") {
    return "/downloads/coresynth-vst3";
  }
  if (product === "standalone") {
    return "/downloads/coresynth-standalone";
  }
  return "";
}
...
function downloadAssetPolicy(assetId) {
  ...
  const table = {
    "coresynth-vst3": { products: ["vst"] },
    "coresynth-standalone": { products: ["standalone"] },
    "coresynth-vst3-a": { products: ["vst"] },
    "coresynth-vst3-b": { products: ["vst"] },
    "coresynth-vst3-c": { products: ["vst"] },
    "coresynth-vst3-d": { products: ["vst"] },
    "coresynth-standalone-a": { products: ["standalone"] },
    "coresynth-standalone-b": { products: ["standalone"] },
    "coresynth-standalone-c": { products: ["standalone"] },
    "coresynth-standalone-d": { products: ["standalone"] },
    "coresynth-linux-amd64.deb": { products: ["vst", "standalone"] },
    "coresynth-linux-arch-x86_64.pkg.tar.zst": { products: ["vst", "standalone"] },
...
function resolveDownloadAsset(assetId) {
  const id = normalizeAssetId(assetId);
  const vstVariant = id.match(/^coresynth-vst3-([abcd])$/i);
  if (vstVariant) {
    ...
    return target
      ? { path: target, fileName: `CoreSynth_v2.2.4${suffix}.vst3`, contentType: "application/octet-stream" }
      : null;
  }
  const standaloneVariant = id.match(/^coresynth-standalone-([abcd])$/i);
  if (standaloneVariant) {
    ...
    return target
      ? {
          path: target,
          fileName: target.endsWith(".exe") ? `coresynth-standalone-v2.2.4${suffix}.exe` : `coresynth-standalone-v2.2.4${suffix}`,
  ...
  if (id === "coresynth-standalone") {
    const target = resolveFirstExisting(buildStandaloneCandidates());
    return target
      ? {
          path: target,
          fileName: target.endsWith(".exe") ? "coresynth-standalone.exe" : "coresynth-standalone",
          contentType: "application/octet-stream"
        }
      : null;
  }
  if (id === "coresynth-vst3") {
    const target = resolveFirstExisting(buildVstCandidates());
    return target
      ? { path: target, fileName: "CoreSynth.vst3", contentType: "application/octet-stream" }
      : null;
  }
  if (id === "coresynth-linux-amd64.deb") {
    if (LINUX_DEB_URL) {
      return { redirect: LINUX_DEB_URL };
    }
    const target = resolveFirstExisting(buildLinuxDebCandidates());
    return target
      ? { path: target, fileName: "coresynth-linux-amd64.deb", contentType: "application/vnd.debian.binary-package" }
      : null;
  }
  if (id === "coresynth-linux-arch-x86_64.pkg.tar.zst") {
    if (LINUX_ARCH_URL) {
      return { redirect: LINUX_ARCH_URL };
    }
    const target = resolveFirstExisting(buildLinuxArchCandidates());
    return target
      ? { path: target, fileName: "coresynth-linux-arch-x86_64.pkg.tar.zst", contentType: "application/zstd" }
      : null;
  }
  return null;
}
...
function parseAdminSessionToken(req) {
  ...
  return cleanCredentialValue(cookies.coresynth_admin_session || "", 260);
}
...
function setAdminSessionCookie(res, token, req) {
  ...
  res.setHeader(
    "Set-Cookie",
    `coresynth_admin_session=${encodeURIComponent(token)}; HttpOnly; Path=/; Max-Age=${maxAgeSec}; SameSite=Lax${secure}`
  );
}
...
async function openaiStructuredChat(prompt, sessionId, memorySummary, options = {}) {
  ...
  if (!cfg.ready) {
    await emitChatWebhook("chat.failed", {
      ...
      error: "OpenAI API key missing — AudioSuite chat unavailable"
    });
    return {
      ok: false,
      error: "OpenAI API key missing — AudioSuite chat unavailable",
  ...
  const personalityMode = cleanText(
    options?.personality_mode || settings.prompt.personality_mode || AudioSuite_BRAIN?.identity?.mode || "professional_mentor",
    80
  );
  const agentModes = normalizeModelList(options?.agent_modes || []);
  const promptMaxChars = Number(settings.context.max_prompt_chars || AudioSuite_BRAIN?.runtime?.max_prompt_chars || 4000);
  const promptText = String(prompt || "").slice(0, promptMaxChars);
  const maxTokens = Number(settings.response.max_output_tokens || AudioSuite_BRAIN?.runtime?.max_tokens || 900);
  ...
  const userPayload = {
    ...
    audiosynth_brain: {
      brain_id: AudioSuite_BRAIN?.brain_id || "audiosuite_brain_v2_2_4",
      version: AudioSuite_BRAIN?.version || "2.2.4",
  ...
  try {
    ...
    if (!response.ok || !rawText) {
      await emitChatWebhook("chat.failed", {
        ...
        error: payload?.error?.message || payload?.message || "OpenAI request failed — AudioSuite chat unavailable"
      });
      return {
        ok: false,
        error: payload?.error?.message || payload?.message || "OpenAI request failed — AudioSuite chat unavailable",
    ...
    await emitChatWebhook("chat.failed", {
      ...
      model,
      attempted_models: [model],
      error: String(error?.message || error) || "OpenAI request failed — AudioSuite chat unavailable"
    });
    return {
      ok: false,
      error: String(error?.message || error) || "OpenAI request failed — AudioSuite chat unavailable",
...
function buildAndroidApkCandidates() {
  const configured = cleanText(process.env.CORESYNTH_ANDROID_APK_PATH || "", 1000);
  const candidates = [
    configured,
...
function buildStandaloneCandidates() {
  const configured = cleanText(process.env.CORESYNTH_STANDALONE_PATH || "", 1000);
  return [
    configured,
    path.join(ROOT_DIR, "dist/coresynth_standalone.exe"),
    path.join(ROOT_DIR, "build/Release/coresynth_standalone.exe"),
    path.join(ROOT_DIR, "build/coresynth_standalone"),
    path.join(ROOT_DIR, "build/coresynth_standalone.exe")
  ].filter(Boolean);
}

function buildVstCandidates() {
  const configured = cleanText(process.env.CORESYNTH_VST_PATH || "", 1000);
  return [
    configured,
    path.join(ROOT_DIR, "dist/CoreSynth.vst3"),
    path.join(ROOT_DIR, "build/Release/CoreSynth.vst3"),
    path.join(ROOT_DIR, "build/CoreSynth.vst3"),
    path.join(ROOT_DIR, "build/coresynth_vst3_stub"),
    path.join(ROOT_DIR, "build/Release/coresynth_vst3_stub.exe"),
    path.join(ROOT_DIR, "build/coresynth_vst3_stub.exe")
  ].filter(Boolean);
}
...
function buildStandaloneVariantCandidates(variant) {
  ...
  const envKey = `CORESYNTH_STANDALONE_PATH_${suffix.toUpperCase()}`;
  const configured = cleanText(process.env[envKey] || "", 1000);
  return [
    configured,
    path.join(ROOT_DIR, `dist/coresynth_standalone_${suffix}.exe`),
    path.join(ROOT_DIR, `dist/coresynth_standalone_${suffix}`),
    path.join(ROOT_DIR, `build/coresynth_standalone_${suffix}.exe`),
    path.join(ROOT_DIR, `build/coresynth_standalone_${suffix}`)
  ].filter(Boolean);
}
...
function buildVstVariantCandidates(variant) {
  ...
  const envKey = `CORESYNTH_VST_PATH_${suffix.toUpperCase()}`;
  const configured = cleanText(process.env[envKey] || "", 1000);
  return [
    configured,
    path.join(ROOT_DIR, `dist/CoreSynth_${suffix.toUpperCase()}.vst3`),
    path.join(ROOT_DIR, `dist/CoreSynth_${suffix}.vst3`),
    path.join(ROOT_DIR, `build/CoreSynth_${suffix.toUpperCase()}.vst3`),
    path.join(ROOT_DIR, `build/CoreSynth_${suffix}.vst3`)
  ].filter(Boolean);
}

function buildLinuxDebCandidates() {
  const configured = cleanText(process.env.CORESYNTH_LINUX_DEB_PATH || "", 1000);
  return [
    configured,
    path.join(ROOT_DIR, "dist/coresynth-linux-amd64.deb"),
    path.join(WEBSITE_DIR, "downloads/coresynth-linux-amd64.deb")
  ].filter(Boolean);
}

function buildLinuxArchCandidates() {
  const configured = cleanText(process.env.CORESYNTH_LINUX_ARCH_PATH || "", 1000);
  return [
    configured,
    path.join(ROOT_DIR, "dist/coresynth-linux-arch-x86_64.pkg.tar.zst"),
    path.join(WEBSITE_DIR, "downloads/coresynth-linux-arch-x86_64.pkg.tar.zst")
  ].filter(Boolean);
}
...
app.get("/api/v1/health", (_req, res) => {
  ...
  res.json({
    ...
    audiosynth_brain_id: AudioSuite_BRAIN?.brain_id || "audiosuite_brain_v2_2_4",
    audiosynth_brain_version: AudioSuite_BRAIN?.version || "2.2.4",
...
app.post("/api/v1/admin/openai/save", requireAdmin, (req, res) => {
  ...
  if (!cfg.ready) {
    res.status(400).json({ ok: false, error: "OpenAI API key missing — AudioSuite chat unavailable", ...buildOpenAiAdminPayload(cfg) });
    return;
  }
  res.json(buildOpenAiAdminPayload(cfg));
});
...
app.get("/downloads/coresynth-standalone", requireDownloadAccess(["standalone"]), (_req, res) => {
  const target = resolveFirstExisting(buildStandaloneCandidates());
  sendDownloadFile(res, target, target && target.endsWith(".exe") ? "coresynth-standalone.exe" : "coresynth-standalone", "application/octet-stream");
});

app.get("/downloads/coresynth-standalone-:variant([a-dA-D])", requireDownloadAccess(["standalone"]), (req, res) => {
  ...
  sendDownloadFile(
    res,
    target,
    target && target.endsWith(".exe") ? `coresynth-standalone-v2.2.4${suffix}.exe` : `coresynth-standalone-v2.2.4${suffix}`,
    "application/octet-stream"
  );
});

app.get("/downloads/coresynth-vst3", requireDownloadAccess(["vst"]), (_req, res) => {
  const target = resolveFirstExisting(buildVstCandidates());
  sendDownloadFile(res, target, "CoreSynth.vst3", "application/octet-stream");
});

app.get("/downloads/coresynth-vst3-:variant([a-dA-D])", requireDownloadAccess(["vst"]), (req, res) => {
  const suffix = String(req.params.variant || "").toLowerCase();
  const target = resolveFirstExisting(buildVstVariantCandidates(suffix)) || resolveFirstExisting(buildVstCandidates());
  sendDownloadFile(res, target, `CoreSynth_v2.2.4${suffix}.vst3`, "application/octet-stream");
});

app.get("/downloads/coresynth-linux-amd64.deb", requireDownloadAccess(["vst", "standalone"]), (_req, res) => {
  if (LINUX_DEB_URL) {
    res.redirect(LINUX_DEB_URL);
    return;
  }
  const target = resolveFirstExisting(buildLinuxDebCandidates());
  sendDownloadFile(res, target, "coresynth-linux-amd64.deb", "application/vnd.debian.binary-package");
});

app.get("/downloads/coresynth-linux-arch-x86_64.pkg.tar.zst", requireDownloadAccess(["vst", "standalone"]), (_req, res) => {
  if (LINUX_ARCH_URL) {
    res.redirect(LINUX_ARCH_URL);
    return;
  }
  const target = resolveFirstExisting(buildLinuxArchCandidates());
  sendDownloadFile(res, target, "coresynth-linux-arch-x86_64.pkg.tar.zst", "application/zstd");
});

app.get("/downloads/coresynth-admin-android.apk", requireAdmin, (req, res) => {
  ...
  sendDownloadFile(res, target, "coresynth-admin-android.apk", "application/vnd.android.package-archive");
});
...
server.listen(PORT, "0.0.0.0", () => {
  console.log(`[coresynth] server listening on :${PORT}`);
});
